...
def test_wanted_only_requests_due_languages(
    ...
    wanted_worker(row, ["provider"])

    assert _captured_languages(generate_calls) == expected


@pytest.mark.parametrize("kind", ["movies", "series"])
def test_wanted_handles_malformed_audio_profile_languages(monkeypatch, wanted_module, row_factory, wanted_worker, kind):
    row = row_factory()
    generate_calls = []
...
@pytest.mark.parametrize("kind", ["movies", "series"])
def test_wanted_uses_none_for_missing_scene_name(monkeypatch, wanted_module, row_factory, wanted_worker, kind):
...
@pytest.mark.parametrize("kind", ["movies", "series"])
def test_wanted_skips_generate_when_path_missing(monkeypatch, wanted_module, row_factory, wanted_worker, kind):
    row = row_factory()
    row.path = None
    generate_calls = []
