...
def test_episode_download_specific_subtitles_uses_none_string_for_missing_scene_name(
    monkeypatch,
    mass_download_module,
    show_row_factory,
    episode_row_factory,
    ...
    module = mass_download_module
    show = show_row_factory(sonarrSeriesId=5, title="Series")
    episode = episode_row_factory(sonarrSeriesId=show.sonarrSeriesId, sonarrEpisodeId=11, sceneName=None)
    generated = []

    monkeypatch.setattr(module.path_mappings, "path_replace", lambda path: path)
    monkeypatch.setattr(module.os.path, "exists", _path_exists)
    monkeypatch.setattr(module, "get_audio_profile_languages", _english_audio_languages)
    monkeypatch.setattr(module, "event_stream", _capture_call([]))
    monkeypatch.setattr(module, "jobs_queue", jobs_queue_factory())
    monkeypatch.setattr(module, "generate_subtitles", _capture_generate_subtitles(generated))
    ...
    assert result == ("", 204)
    assert generated[0][0][3] == "None"


def test_movie_download_specific_subtitles_uses_none_string_for_missing_scene_name(
    monkeypatch,
    mass_download_module,
    movie_row_factory,
    jobs_queue_factory,
    ...
    module = mass_download_module
    movie = movie_row_factory(sceneName=None)
    generated = []
    ...
    monkeypatch.setattr(module, "event_stream", _capture_call([]))
    monkeypatch.setattr(module, "jobs_queue", jobs_queue_factory())
    monkeypatch.setattr(module, "get_profile_id", lambda movie_id: 11)
    monkeypatch.setattr(module, "generate_subtitles", _capture_generate_subtitles(generated))

    result = module.movie_download_specific_subtitles(movie.radarrId, "fr", "False", "False", job_id="job")
