...
def test_wanted_handles_multi_colon_language_codes(
    monkeypatch, kind, wanted_module, row_factory, wanted_worker
):
...
def test_wanted_normalizes_and_deduplicates_missing_languages(
    monkeypatch, kind, wanted_module, row_factory, wanted_worker, missing_subtitles, expected
):
    module = wanted_module
    item = row_factory()
...
def test_wanted_chains_failed_attempt_updates_across_languages(
    monkeypatch, kind, wanted_module, row_factory, wanted_worker, missing_subtitles, expected_calls
):
    module = wanted_module
    item = row_factory()
    item.missing_subtitles = missing_subtitles
...
def test_wanted_stamps_canonical_language_tokens(monkeypatch, kind, wanted_module, row_factory, wanted_worker):
    module = wanted_module
    item = row_factory()
