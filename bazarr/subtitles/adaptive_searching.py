    ...
    for desired_language in desired_languages:
        ...
        if debug_logging:
            try:
                ...
                logging.debug("Adaptive searching: unable to format initial and latest search timestamps for logging.")
                initial_search_timestamp = None
                latest_search_timestamp = None

            if initial_search_timestamp is not None:
                logging.debug(f"Adaptive searching: initial search date for {desired_language} is {initial_search_timestamp}")
            if latest_search_timestamp is not None:
